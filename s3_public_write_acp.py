...
def handler(event, context):
    ...
    for account in craws.accounts:
        try:
            ...
            response = s3_client.head_object(Bucket = craws.bucket, Key = today+'/'+account['account_id']+'/S3PublicWriteAcp.json')
            ...
            results['Description'] = 'Granting public &#39;WRITE_ACP&#39; access to your S3 buckets can allow anonymous users to ' +\
                                    'edit their ACL permissions and eventually be able to view, upload, modify and delete S3 ' +\
                                    'objects within the bucket without restrictions, which can lead to data loss or economic ' +\
                                    'denial-of-service attacks (i.e. uploading a large number of files to drive up the costs of ' +\
                                    'the S3 service within your AWS account).'
            details = []
            try:
